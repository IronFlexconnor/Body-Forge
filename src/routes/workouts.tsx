  ...
  if (active)
    return (
      <ActiveSession
        ...
        onComplete={() => {
          setActive(null);
          setUpcoming((u) => u.filter((w) => w.id !== active.id));
        }}
      />
    );
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="font-semibold leading-tight truncate">
                {currentGoal ?? "Pick a goal"}
              </div>
            </div>
            <Button
              onClick={() => setShowGoals((s) => !s)}
              size="sm"
              className="h-9 rounded-full bg-gradient-primary text-xs font-semibold text-primary-foreground shadow-glow"
            >
          ...
          {showGoals && (
            <div className="mt-4">
              <p className="mb-3 text-xs text-muted-foreground">
                Tap a goal — Coach instantly rebuilds your program around it.
              </p>
              <GoalSelector
                compact
                onBuilt={() => {
...
function ActiveSession({
  ...
  const [logs, setLogs] = useState<
    Record<string, { reps: string; weight: string; rpe: string; done: boolean }[]>
  >({});
  ...
  useEffect(() => {
    ...
    workout.exercises.forEach((ex) => {
      init[ex.name] = Array.from({ length: ex.sets || 3 }, () => ({
        reps: "",
        weight: "",
        rpe: "",
        done: false,
    ...
    (async () => {
      ...
      const bestMap: Record<string, { bestWeight: number; bestE1RM: number }> = {};
  ...
  const toggleDone = async (ex: string, idx: number) => {
    ...
    if (next && (set.reps || set.weight)) {
      ...
      // Auto-start the rest timer using the exercise's prescribed rest
      const exDef = workout.exercises.find((e) => e.name === ex);
      setRestExercise(ex);
      setRestSeconds(exDef?.rest_sec && exDef.rest_sec > 0 ? exDef.rest_sec : 90);
      setRestKey((k) => k + 1);

      // PR detection against historical bests (celebrated instantly, saved async)
      if (weight != null && reps != null && weight > 0 && reps > 0) {
        const pr = detectPR(weight, reps, bestByExercise[ex] ?? null);
        if (pr.isPR) {
          setBestByExercise((m) => ({
            ...m,
            [ex]: {
              bestWeight: Math.max(weight, m[ex]?.bestWeight ?? 0),
              bestE1RM: pr.newBestE1RM,
            },
          }));
          if (!pr.isBaseline) {
            setPrCount((c) => c + 1);
            ...
            toast.success(
              pr.kind === "weight"
                ? `🏆 New ${ex} PR — ${weight}${weightUnit} is your heaviest ever!`
                : `🏆 Rep PR on ${ex} — ${weight}${weightUnit} × ${reps} beats your best!`,
              { duration: 4000 },
            );
            supabase
              ...
              .insert({
                ...
                meta: {
                  ...
                  reps,
                  kind: pr.kind,
                  e1rm: Math.round(pr.newBestE1RM * 10) / 10,
                },
              })
              .then(() => {});
          }
      ...
      // Un-checking a set dismisses the timer it started
      setRestKey(0);
    }
  };

  const addSet = (ex: string) =>
    setLogs((l) => ({ ...l, [ex]: [...l[ex], { reps: "", weight: "", rpe: "", done: false }] }));
  const removeSet = (ex: string, idx: number) =>
    setLogs((l) => ({ ...l, [ex]: l[ex].filter((_, i) => i !== idx) }));
  ...
  const finish = async () => {
    ...
    supabase.functions
      .invoke("auto-adjust", { body: { trigger: "workout_complete", workout_log_id: logId } })
      .then(({ data }) => {
        const d = data as any;
  ...
  return (
    <div className="min-h-dvh bg-background pb-32">
                ...
                <div className="min-w-0">
                  ...
                  {last ? (
                    <div className="mt-1 inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-semibold text-primary">
                      Last: {last.weight}
                      {last.unit ?? weightUnit} × {last.reps ?? "—"}
                    </div>
                  ) : (
                    <div className="mt-1 text-[10px] text-muted-foreground">
                      First time logging — set your baseline 💪
                    </div>
                  )}
                </div>
                <div className="ml-3 flex shrink-0 gap-1.5">
                  <button
                    onClick={() => {
                      ...
                      const w = current
                        ? parseFloat(current)
                        : (rec?.nextWeight ?? last?.weight ?? null);
                      setPlateTarget({
                        weight: w != null && Number.isFinite(Number(w)) ? Number(w) : null,
                      });
                    }}
                    ...
                    aria-label={`Plate calculator for ${ex.name}`}
                  >
                    <Disc3 className="h-4 w-4" />
                  </button>
                  <button
                    onClick={() => analyzeExercise(ex.name)}
                    className="grid h-9 w-9 place-items-center rounded-xl border border-primary/30 bg-primary/10 text-primary transition-colors hover:border-primary"
                    aria-label={`Analyze ${ex.name} form`}
                  >
                ...
                <div className="mb-3 flex items-start gap-2 rounded-xl border border-primary/30 bg-gradient-to-r from-primary/15 to-primary/5 p-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-sm font-semibold leading-tight">
                      {rec.nextWeight}
                      {weightUnit} × {rec.topReps || ex.reps}
                      <span className="ml-1.5 text-[11px] font-medium text-muted-foreground">
                        (last: {rec.topWeight}
                        {weightUnit} × {rec.topReps}
                        {rec.rpe != null ? ` @ RPE ${rec.rpe}` : ""})
                      </span>
                    </div>
              ...
              <div className="space-y-2">
                ...
                {logs[ex.name]?.map((s, i) => (
                  <div
                    key={i}
                    className={cn(
                      "grid grid-cols-[24px_1fr_1fr_1fr_36px] items-center gap-2 rounded-lg p-1 transition-colors",
                  ...
                  >
                    ...
                    <Input
                      inputMode="decimal"
                      aria-label={`${ex.name} set ${i + 1} weight in ${weightUnit}`}
                      value={s.weight}
                      onChange={(e) => updateSet(ex.name, i, "weight", e.target.value)}
                      placeholder={
                        rec?.nextWeight != null
                          ? String(rec.nextWeight)
                          : last?.weight != null
                            ? String(last.weight)
                            : weightUnit
                      }
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  {logs[ex.name]?.length > 1 && (
                    <button
                      onClick={() => removeSet(ex.name, logs[ex.name].length - 1)}
                      aria-label={`Remove last ${ex.name} set`}
                      className="rounded-lg border border-dashed border-border px-3 py-2 text-xs font-medium text-muted-foreground hover:text-destructive"
                    >
      ...
      {plateTarget && (
        <PlateCalculator
          initialWeight={plateTarget.weight}
          unit={weightUnit}
          ...
          <div className="relative max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-3xl border border-border/60 bg-gradient-card shadow-card sm:rounded-3xl">
            <div className="px-6 pt-7 pb-3 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {summary.durationMin} min · {summary.totalSets} sets ·{" "}
                {Math.round(summary.totalVolume).toLocaleString()} {weightUnit} of total volume
                {prCount > 0 && (
            ...
            <div className="px-6 pt-2 pb-6 space-y-4">
              <div>
                ...
                {summary.recs.length === 0 ? (
                  <p className="rounded-xl border border-dashed border-border/60 p-4 text-center text-xs text-muted-foreground">
                    Log weight + reps on at least one set next time to unlock per-lift
                    recommendations.
                  </p>
                ) : (
                  <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                ...
                <Button
                  className="h-11 rounded-xl bg-gradient-primary font-semibold text-primary-foreground shadow-glow"
