    ...
    <div className="min-h-dvh bg-gradient-hero px-6 py-16">
      <div className="mx-auto max-w-md text-center">
        <div className="mx-auto mb-6 grid h-20 w-20 place-items-center rounded-full bg-gradient-primary text-primary-foreground shadow-glow">
          {isActive ? <CheckCircle2 className="h-10 w-10" /> : <Loader2 className="h-10 w-10 animate-spin" />}
        </div>
        <h1 className="text-3xl font-bold tracking-tight">
          {isActive ? <>You're <span className="text-gradient-primary">in</span>.</> : "Setting up your access…"}
        </h1>
        ...
        {session_id && <p className="mt-4 text-[10px] text-muted-foreground/60">Ref: {session_id.slice(-8)}</p>}
        <Button
          onClick={() => navigate({ to: "/" })}
          disabled={loading}
