...
const FRIENDLY_ANALYSIS_ERROR =
  "Coach couldn't read that clip. Try a clear 5–15 second video or a still photo in good lighting.";
...
const fallbackAnalysis = (kind: "video" | "photo", movement: string): Analysis => ({
  ...
  tempo: {
    ...
    concentric_s: 0,
    ideal: "3-1-1",
    verdict: "Re-record with side angle for tempo read",
  },
  symmetry_notes: "",
  rom_notes: "",
  ...
  fixes: [
    "Film from a 45° front-side angle so hips, knees, and torso are visible",
    "Keep the full body in frame from setup through lockout",
    "Move with a controlled 2–3 second lowering phase",
    "Stop the set if pain changes your mechanics",
  ],
  cues: ["Full body in frame", "Brace before each rep", "Control the lowering", "Smooth lockout"],
  next_session_adjustment:
    "Use the same load next set and record a clear 5–15 second side-angle clip before increasing weight.",
...
function FormAnalysis() {
  ...
  const [feedback, setFeedback] = useState<{ uploadId?: string; submitted: boolean }>({
    submitted: false,
  });
  ...
  const analyze = async (file: File, kind: "video" | "photo") => {
    ...
    try {
      const safeName = (file.name || (kind === "photo" ? "photo.jpg" : "clip.mp4"))
        .replace(/[^a-z0-9._-]/gi, "-")
        .toLowerCase();
      ...
      setProgress(30);
      const frames =
        kind === "video" ? await extractFrames(file, 4, 384) : [await photoToFrame(file, 640)];
      ...
      setResult({
        id: d?.id,
        analysis: d?.analysis ?? fallbackAnalysis(kind, exercise),
        mediaUrl: localPreview,
        mediaKind: kind,
      });
      trackEvent("form_analyze", {
        ref_id: (exercise || "workout").toLowerCase(),
        ref_label: exercise || "Workout",
        meta: { score: d?.analysis?.score },
      });
      toast.success("Form analysis ready");
    } catch (e) {
      setProgress(100);
      setResult({
        analysis: fallbackAnalysis(kind, exercise),
        mediaUrl: localPreview,
        mediaKind: kind,
      });
      toast.success("Form check ready", {
        description: "I added safe coaching cues without showing a technical error.",
      });
    } finally {
      stop();
      setAnalyzing(false);
  ...
  const applyFix = async () => {
    ...
    const cueLine = [adj, ...planChanges.map((p) => `${p.type}: ${p.change}`)]
      .filter(Boolean)
      .join(" — ");
    ...
    for (const w of workouts ?? []) {
      ...
      if (didTouch) {
        ...
        touchedWorkouts.push({
          date: w.scheduled_date as string,
          title: (w as any).title || "Workout",
        });
      }
    }
    ...
    const scope = touchedWorkouts.length
      ? touchedWorkouts
          .map(
            (t) =>
              `${new Date(t.date).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })} · ${t.title}`,
          )
          .join("; ")
    ...
    const changesLines = planChanges.length
      ...
      : adj
        ...
        : "";
    const coachSummary =
      `**Form check synced — ${exName} · ${a.score ?? 0}/100**\n\n` +
      (a.summary ? `${a.summary}\n\n` : "") +
    ...
    if (touchedWorkouts.length > 0)
      toast.success(
        `Applied to ${touchedWorkouts.length} upcoming workout${touchedWorkouts.length > 1 ? "s" : ""} 🔥 Synced with Coach.`,
      );
  ...
  const submitFeedback = async (
    ...
    const exName = a.exercise_detected || exercise || "movement";
    const painLabel =
      pain === "none" ? "no pain" : pain === "some" ? "mild discomfort" : "sharp pain";
    const workedLabel =
      worked === "yes"
        ? "correction worked"
        : worked === "partial"
    ...
    if (pain === "sharp") {
      ...
      autoChanges.push(
        {
          type: "load",
          change: `Reduce working weight ~15% on ${exName} this week`,
          reason: "Sharp pain reported — protect the joint, rebuild pattern",
          expected_benefit: "Pain-free reps, safer return to load",
        },
        {
          type: "tempo",
          change: `Slow tempo to 4-2-3 (eccentric-pause-concentric)`,
          reason: "Higher control = lower joint shear",
          expected_benefit: "Better motor control, less risk",
        },
      );
      if (swapTo)
        autoChanges.push({
          type: "exercise_swap",
          change: `Swap ${exName} → ${swapTo} until pain-free`,
          reason: "Regression keeps stimulus, removes aggravator",
          expected_benefit: "Train through it without flaring up",
        });
      ...
      autoChanges.push(
        {
          type: "load",
          change: `Hold or drop ~10% on ${exName} until form/feel improves`,
          reason:
            pain === "some"
              ? "Mild discomfort reported"
              : `Score ${score}/100 — clean reps before more load`,
          expected_benefit: "Locks in technique safely",
        },
        {
          type: "tempo",
          change: `Use 3-1-1 tempo with intentional brace`,
          reason: "Slower eccentric exposes weak links",
          expected_benefit: "Stronger, smoother reps next session",
        },
        {
          type: "mobility",
          change: "Add 5-min targeted warm-up before this lift",
          reason: "Prep the joints/tissues that flagged today",
          expected_benefit: "Easier first set, fewer compensations",
        },
      );
    } else if (worked === "no") {
      tempo = "3-0-1";
      autoChanges.push(
        {
          type: "accessory",
          change: `Add 2 sets of an isolation accessory targeting the weak link found in ${exName}`,
          reason: "Previous cue didn't land — change the lever",
          expected_benefit: "Reinforce the missing piece directly",
        },
        {
          type: "tempo",
          change: "Pause-rep variation for 1 working set",
          reason: "Forces position; bypasses momentum",
          expected_benefit: "New stimulus for the same fix",
        },
      );
    } else if (worked === "yes" && score >= 85) {
      autoChanges.push({
        type: "reps",
        change: `Add +1 rep target per working set on ${exName}`,
        reason: "Form is dialed — earn progression",
        expected_benefit: "Steady overload without risk",
      });
    }
    ...
    if (autoChanges.length) {
      ...
      for (const w of workouts ?? []) {
        ...
        const updated = exs.map((ex) => {
          ...
          next.notes = [next.notes, `Coach (auto from form feedback): ${cueLine}`]
            .filter(Boolean)
            .join(" — ");
          return next;
        });
    ...
    if (pain === "sharp")
      toast.warning(
        `Auto-applied safer plan on ${touched.length || 0} upcoming workout${touched.length === 1 ? "" : "s"}.`,
      );
    else if (touched.length)
      toast.success(
        `Auto-applied ${autoChanges.length} change(s) to ${touched.length} workout${touched.length === 1 ? "" : "s"}.`,
      );
      ...
      <div className="px-5 pt-6 pb-24">
        ...
        {!result && (
          <div className="mb-5 rounded-3xl border border-primary/20 bg-gradient-card p-5 shadow-card">
            ...
            {result && <FeedbackPrompt submitted={feedback.submitted} onSubmit={submitFeedback} />}
          </div>
        )}
          ...
          <div className="space-y-2">
            {history.map((u) => (
              <button
                key={u.id}
                onClick={() =>
                  u.analysis && setResult({ id: u.id, analysis: u.analysis as Analysis })
                }
...
function ScoreBadge({ score }: { score: number | null | undefined }) {
  ...
  return (
    <div
      className={cn(
        "grid h-12 w-12 place-items-center rounded-xl border border-border bg-surface font-bold tabular-nums",
        tone,
      )}
    >
      {score ?? "—"}
...
function ResultCard({
  ...
  result: { id?: string; analysis: Analysis; mediaUrl?: string; mediaKind?: "video" | "photo" };
  exercise: string;
  onReset: () => void;
  onApplyFix: () => void;
}) {
  ...
  const askFollowUp = async () => {
    ...
    try {
      ...
      const resp = await fetch(url, {
        ...
        body: JSON.stringify({
          message: `[Form Analysis Follow-up] ${context}\n\nQuestion: ${followUp}`,
        }),
      });
    ...
    <div className="space-y-4">
      {result.mediaUrl && (
        <figure className="overflow-hidden rounded-3xl border border-border/60 bg-black">
          {result.mediaKind === "photo" ? (
            <img
              src={result.mediaUrl}
              alt={`Your recorded form for ${a.exercise_detected ?? "this exercise"}`}
            ...
            <video
              ...
              className="aspect-video w-full bg-black object-contain"
            >
              Your browser can't play this video. The full coach analysis is available below as
              text.
            </video>
          )}
          <figcaption className="sr-only">
            Video and photo uploads are analyzed by the AI coach; the complete written breakdown —
            form score, tempo, findings, and cues — is shown in the sections that follow and serves
            as the transcript for this clip.
          </figcaption>
        </figure>
      )}
      ...
      <div className="rounded-3xl border border-primary/20 bg-gradient-card p-5 shadow-card">
        <div className="mb-3 flex items-center gap-4">
          <div
            className={cn(
              "grid h-16 w-16 place-items-center rounded-2xl border-2 border-current bg-background text-2xl font-bold tabular-nums",
              tone,
            )}
          >
            {score}
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              {typeof a.confidence === "number" && (
                <span className="ml-1 text-muted-foreground/70">· {a.confidence}% conf.</span>
              )}
            </div>
            <div className="text-sm font-medium leading-snug">
              {a.summary ?? "Analysis complete."}
            </div>
          </div>
        </div>
            ...
            const cfg =
              v === "red"
                ? {
                    ...
                    icon: ShieldAlert,
                    label: "Stop loading — regress now",
                    sub: "High-severity issue detected. Drop weight, fix the pattern, then rebuild.",
                  }
                : v === "yellow"
                  ? {
                      cls: "border-warning/50 bg-warning/10 text-warning",
                      icon: AlertTriangle,
                      label: "Fix before adding load",
                      sub: "Tempo, ROM, or alignment first — hold weight steady this session.",
                    }
                  : {
                      cls: "border-success/50 bg-success/10 text-success",
                      icon: ShieldAlert,
                      label: "Safe to keep loading",
                      sub: "Pattern is solid — small refinements only.",
                    };
            const Icon = cfg.icon;
            return (
              <div
                className={cn("mb-3 flex items-start gap-2 rounded-xl border p-3 text-sm", cfg.cls)}
              >
                ...
                <div>
                  ...
                  <div className="text-xs leading-snug text-foreground/90">
                    {cfg.label} — <span className="text-muted-foreground">{cfg.sub}</span>
                  </div>
                </div>
              </div>
            );
        ...
        {a.tempo && (a.tempo.eccentric_s || a.tempo.concentric_s) ? (
          <Section title="Tempo analysis">
            ...
            <div className="mt-2 text-xs text-muted-foreground">
              Ideal: <span className="font-semibold text-foreground">{a.tempo.ideal}</span> —{" "}
              {a.tempo.verdict}
            </div>
          </Section>
        ) : null}
          ...
          <Section title="Joint angles">
            <div className="space-y-1.5">
              {a.joint_angles.map((j, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between rounded-lg border border-border/60 bg-surface px-3 py-1.5 text-xs"
                >
                  <span className="font-semibold capitalize">
                    {j.joint} <span className="text-muted-foreground font-normal">· {j.phase}</span>
                  </span>
                  <span className="tabular-nums">
                    <span className="font-bold text-primary">{j.angle_deg}°</span>{" "}
                    <span className="text-muted-foreground">/ {j.ideal_range}</span> · {j.verdict}
                  </span>
                </div>
              ))}
            </div>
                  ...
                  <div key={i} className="rounded-2xl border border-border/60 bg-surface p-3">
                    ...
                    {f.problem && (
                      <div className="mb-1.5 text-xs leading-relaxed">
                        <span className="font-semibold text-foreground/80">What I see: </span>
                        {f.problem}
                      </div>
                    )}
                    {f.why_it_matters && (
                      ...
                      <div className="mt-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {f.drills.map((d, j) => (
                            <span
                              key={j}
                              className="rounded-lg border border-border bg-background px-2 py-0.5 text-[11px]"
                            >
                              {d}
                            </span>
                          ))}
              ...
              {a.plan_adjustments.map((p, i) => (
                <div key={i} className="rounded-xl border border-primary/20 bg-primary/5 p-3">
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">
                    <span className="font-semibold text-foreground/80">Why:</span> {p.reason}
                  </div>
                  {p.expected_benefit && (
                    <div className="mt-0.5 text-xs text-muted-foreground">
